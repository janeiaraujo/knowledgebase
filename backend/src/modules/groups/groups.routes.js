...
export default async function groupRoutes(fastify) {
    ...
    // Create group (admin/owner only)
    fastify.post('/', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner']), auditMiddleware('group_created')]
    }, async(request, reply) => {
        const db = fastify.db();
    ...
    fastify.put('/:id', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner']), auditMiddleware('group_updated')]
    }, async(request, reply) => {
        const db = fastify.db();
        const id = toObjectId(request.params.id);
        const { name, description, department_id, parent_group_id } = request.body;
        ...
        await db.collection('groups').updateOne({ _id: id, tenant_id: request.tenantId }, { $set: updateData });
    ...
    fastify.delete('/:id', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner']), auditMiddleware('group_deleted')]
    }, async(request, reply) => {
        const db = fastify.db();
        const id = toObjectId(request.params.id);
    ...
    fastify.post('/:id/users', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner']), auditMiddleware('user_added_to_group')]
    }, async(request, reply) => {
        const db = fastify.db();
    ...
    // Remove user from group (admin/owner only)
    fastify.delete('/:id/users/:user_id', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner']), auditMiddleware('user_removed_from_group')]
    }, async(request, reply) => {
        const db = fastify.db();
        const groupId = toObjectId(request.params.id);
