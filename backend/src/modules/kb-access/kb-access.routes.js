...
export default async function kbAccessRoutes(fastify) {
    ...
    // Set KB access control (admin/owner/reviewer only)
    fastify.post('/:kb_id', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner', 'reviewer']), auditMiddleware('kb_access_updated')]
    }, async(request, reply) => {
        const db = fastify.db();
        const kbId = toObjectId(request.params.kb_id);
        ...
        await db.collection('kb_access').updateOne({ kb_id: kbId, tenant_id: request.tenantId }, {
            $set: accessData,
    ...
    // Delete KB access control (admin/owner only) - resets to default restricted
    fastify.delete('/:kb_id', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner']), auditMiddleware('kb_access_deleted')]
    }, async(request, reply) => {
        const db = fastify.db();
        const kbId = toObjectId(request.params.kb_id);
    ...
    // Bulk set access for multiple KBs (admin/owner only)
    fastify.post('/bulk', {
        preHandler: [authMiddleware, tenantMiddleware, requireRole(['admin', 'owner']), auditMiddleware('kb_access_bulk_updated')]
    }, async(request, reply) => {
        const db = fastify.db();
